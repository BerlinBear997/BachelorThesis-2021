...
#include <unistd.h>
	...
	#define MSR_CORE_FREQ 0x199
        ...
        #define MAX_CORE_FREQ 28
        #define MIN_CORE_FREQ 12
        #define CORE_FREQ_STEP 1
#endif
...
int main(int argc, char *argv[]) {
	...
	int freq=MAX_CORE_FREQ; 
	if(argc == 2) {
		freq = atoi( argv[1] );	
	}
	if(freq <MIN_CORE_FREQ || freq>MAX_CORE_FREQ) {
	...
	for(i=0; i<36; i++) {
		...
		//printf("core freq %x \n", tmp);
		write_msr_by_coord(0, i, 0, MSR_CORE_FREQ, 0x1400);
	...
	for(i=0; i<36; i++) {
		...
		//printf("core freq %x \n", tmp);
		write_msr_by_coord(1, i, 0, MSR_CORE_FREQ, 0x1400);
